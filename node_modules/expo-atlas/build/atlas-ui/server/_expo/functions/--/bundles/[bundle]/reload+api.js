...
__d((function(g,r,i,a,m,e,d){Object.defineProperty(e,"__esModule",{value:!0}),e.GET=async function(n,o){try{const n=await(0,s.getSource)().getBundle(o.bundle);if(!n)return Response.json({error:'Bundle not found'},{status:404});if(!n.serializeOptions?.sourceUrl)return Response.json({error:'Bundle has no `serializeOptions.sourceUrl`'},{status:406});const t=new URL(n.serializeOptions.sourceUrl);return t.hostname='localhost',Response.redirect(t,302)}catch(s){return Response.json({error:s.message},{status:406})}};var s=r(d[0])}),1698,[1692]);
module.exports = __r(1698);